...
# ===== إضافة: دالة إعادة التشغيل التلقائي =====
def auto_restart_on_crash(func):
    """ديكوراتور لإعادة التشغيل التلقائي عند أي خطأ"""
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"⚠️ محاولة {attempt + 1}/{max_retries} فشلت بسبب: {e}")
                
                # إعادة تشغيل البرنامج
                if attempt < max_retries - 1:
                    print(f"🔄 إعادة التشغيل بعد {retry_delay} ثواني...")
...
# ===== إضافة: دالة مراقبة الذاكرة =====
def monitor_memory_usage():
    """مراقبة استخدام الذاكرة لمنع الإيقاف"""
    import psutil
    ...
    while True:
        try:
            ...
            # إذا تجاوزت الذاكرة 70%، تنظيف المؤقت
            if memory_percent > 70:
                print(f"⚠️ استخدام الذاكرة مرتفع: {memory_percent:.1f}%")
            ...
            # في حالة أي خطأ، الاستمرار دون إيقاف
            time.sleep(30)

# ===== إضافة: دالة تنظيف الملفات المؤقتة =====
def cleanup_temp_files():
    """تنظيف الملفات المؤقتة القديمة"""
    try:
        current_time = time.time()
        for filename in os.listdir('.'):
            if filename.startswith(('temp_', 'full_results_')):
...
# ===== إضافة: دالة إعادة الاتصال بالإنترنت =====
def check_internet_connection():
    """فحص الاتصال بالإنترنت وإعادة المحاولة"""
    max_retries = 5
    retry_delay = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"⚠️ فشل الاتصال بالإنترنت، المحاولة {attempt + 1}/{max_retries}")
            time.sleep(retry_delay)
            retry_delay *= 2
...
def install_missing_packages():
    ...
    required_packages = [
        ...
        'telebot',
        'requests',
        'psutil'  # إضافة psutil لمراقبة الذاكرة
    ]
...
# ===== إضافة: بدء مراقبة الذاكرة في خيط منفصل =====
try:
    memory_monitor_thread = Thread(target=monitor_memory_usage, daemon=True)
    memory_monitor_thread.start()
    print("✅ بدء مراقبة الذاكرة...")
except:
    print("⚠️ تعذر بدء مراقبة الذاكرة")
...
# ===== إضافة: دالة تشغيل البوت مع استمرارية =====
@auto_restart_on_crash
def run_bot():
    """تشغيل البوت مع استمرارية"""
    print("="*50)
    ...
    print("✅ تم إضافة نظام الاستمرارية ومنع التوقف")
    print("="*50)
    ...
    if not check_internet_connection():
        print("❌ فشل الاتصال بالإنترنت، إعادة المحاولة...")
        time.sleep(10)
        os.execl(sys.executable, sys.executable, *sys.argv)
    ...
    try:
        ...
        raise  # سيتم التعامل معه من خلال الديكوراتور auto_restart_on_crash

# ===== تشغيل البوت مع الاستمرارية =====
